...
VIRTUAL CLWrapper *ActivationLayer::getGradInputWrapperTest() {
#if TRANSFERCL_VERBOSE == 1
...
VIRTUAL CLWrapper *ActivationLayer::getOutputWrapperTest() {
#if TRANSFERCL_VERBOSE == 1
LOGI( "DeepCL/src/activate/ActivationLayer.cpp: getOutputWrapper");
#endif
...
//if (typeid(previousLayer) == typeid(ConvolutionalLayer))
	...
	#endif
#endif
//LOGI("-----------------in ActivationLayer: min %f mult: %f",min,mult);
...
gradInputWrapperHalf=nextLayer->getGradInputWrapperTest();
...
VIRTUAL CLWrapper * ActivationLayer::getOutput8bitInfo(float &_min,float &_mult){
#if TRANSFERCL_VERBOSE == 1
LOGI( "DeepCL/src/conv/ActivationLayer.cpp: getOutputWrapper");
#endif
	_min=min;
	_mult=mult;
