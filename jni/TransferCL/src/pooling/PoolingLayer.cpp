...
//    }
   ...
   if(not setup){
		...
		gradInputWrapperHalf->createZeroCopyObject_ReadFlag_OnDevice();//->createZeroCopyObject_HostNotAccessFlag_OnDevice();
		setup=true;
	}
#endif
}
...
VIRTUAL CLWrapper *PoolingLayer::getGradInputWrapperTest() {
#if TRANSFERCL_VERBOSE == 1
LOGI( "DeepCL/src/pooling/PoolingLayer.cpp: getGradInputWrapper");
#endif
...
VIRTUAL CLWrapper *PoolingLayer::getOutputWrapperTest() {
#if TRANSFERCL_VERBOSE == 1
LOGI( "DeepCL/src/pooling/PoolingLayer.cpp: getOutputWrapper");
#endif
...
LOGI( "DeepCL/src/pooling/PoolingLayer.cpp: forward");
	...
	outputWrapperTest= previousLayer->getOutputWrapperTest();
	selectorsWrapper= previousLayer->getSelectorWrapper();
	#if TRANSFER==1
		#if EIGHT_BIT_ACCURACY==1
			outputWrapper8Bit=previousLayer->getOutput8bitInfo(min,mult);
		#endif
	#endif
#endif
//LOGI("-----------------in PoolingLayer: min %f mult: %f",min,mult);
//
//if (1){
//outputWrapper= previousLayer->getOutputWrapper();
...
//
	...
	#if TEST_FORWARD_PROP == 0
		if (dynamic_cast<ConvolutionalLayer*>(previousLayer->previousLayer))
			inputWrapper=(dynamic_cast<ConvolutionalLayer*>(previousLayer->previousLayer))->gradInput_poolingLayer_WrapperHalf;
		else
			if (dynamic_cast<ConvolutionalLayer*>(previousLayer))
				inputWrapper=(dynamic_cast<ConvolutionalLayer*>(previousLayer))->gradInput_poolingLayer_WrapperHalf;
			//inputWrapper=((dynamic_cast<ConvolutionalLayer*>(previousLayer->previousLayer))->gradInputWrapper);
	#endif
	poolingBackpropImpl->backwardHalf(batchSize, gradOutputWrapperHalf, selectorsWrapper, gradInputWrapperHalf, inputWrapper);
#else
	#if TEST_FORWARD_PROP == 0
		if (dynamic_cast<ConvolutionalLayer*>(previousLayer->previousLayer))
...
VIRTUAL CLWrapper * PoolingLayer::getOutput8bitInfo(float &_min,float &_mult){
#if TRANSFERCL_VERBOSE == 1
LOGI( "DeepCL/src/conv/PoolingLayer.cpp: getOutputWrapper");
#endif
	_min=min;
	_mult=mult;
