...
VIRTUAL void PoolingBackwardGpuNaive::backwardHalf(int batchSize, CLWrapper *gradOutputWrapper, CLWrapper *selectorsWrapper,
        CLWrapper *gradInputWrapper,CLWrapper * inputWrapper){
	#if MEASURE_BACKWARD_PROP == 1
		LOGE("backward pooling");
		//inputWrapper->copyToDevice();// problem it seems th buffer is durty. Check why ?
		LOGI("isOnDevice %d",inputWrapper->isDeviceDirty());
	#endif
    kernel3->in(batchSize)->inout(gradOutputWrapper)->in(selectorsWrapper)->in(gradInputWrapper)->in(inputWrapper);
    if (not setup) {
		workgroupSize2 = kernel3->get_kernel_work_group_size();;
		numWorkgroups2 = (batchSize * numPlanes * outputSize * outputSize + workgroupSize2 - 1) / workgroupSize2;
		setup= true;
	}
	#if MEASURE_BACKWARD_PROP == 1
		LOGI("globalSize %d workgroupsize %d",numWorkgroups2 * workgroupSize2,workgroupSize2);
	#endif
    kernel3->run_1d(numWorkgroups2 * workgroupSize2, workgroupSize2);
    cl->finish();
	#if MEASURE_BACKWARD_PROP == 1
		if (test){
			inputWrapper->copyToHost();
...
PoolingBackwardGpuNaive::PoolingBackwardGpuNaive(EasyCL*cl, bool padZeros, int numPlanes, int inputSize, int poolingSize,int previousLayer_activationLayer,bool bool_test) :
    ...
    kernelSource2 ="#pragma OPENCL EXTENSION cl_khr_fp16 : enable\n\n"
		"kernel void backward_PoolingHalf(const int batchSize,\n"
        "    const __global half* restrict gradOutput, const __global int* restrict selectors, global half *gradInput,const __global half* restrict inputs) {\n"
        "\n"
        "    int globalId=get_global_id(0);\n"
        "    int nPlaneCombo=(globalId / {{gOutputSizeSquared}});\n"
        "    int outputPosCombo=(globalId % {{gOutputSizeSquared}});\n"
        ...
        "\n"
        "    int resultIndex = (( n* {{gNumPlanes}} + plane)* {{gOutputSize}} + outputRow)* {{gOutputSize}} + outputCol;\n"
        "    half error={{gActivationFunction}}\n"
        "    int selector = (selectors[resultIndex]);\n"
        "    int drow=(selector / {{gPoolingSize}});\n"
        "    int dcol=(selector % {{gPoolingSize}});\n"
    	"{{gPoolingBackprop}}"
    ...
    buildKernelBackwardHalf(kernelSource2,previousLayer_activationLayer);

}

void PoolingBackwardGpuNaive::buildKernelBackwardHalf( string kernelSource,int previousLayer_activationLayer) {
    TemplatedKernel builder(cl);


        setupBuilderBackwardHalf(&builder,previousLayer_activationLayer);

        //string identifier2="backward_Pooling"+std::to_string(inputSize);

    	string identifier2="backward_PoolingHalf";
        ...
        this->kernel3 = builder.buildKernel(
           		identifier2,
               "backward_PoolingHalf",
               kernelSource.c_str(),
               "backward_PoolingHalf",
               false
        );
...
void PoolingBackwardGpuNaive::setupBuilderBackwardHalf(TemplatedKernel *builder,int previousLayer_activationLayer){
...
void  PoolingBackwardGpuNaive::setActivationFunctionHalf(TemplatedKernel *builder,int previousLayer_activationLayer){
	...
	string replaceString=" ";
		...
		if (previousLayer_activationLayer==1)
			replaceString=  "(gradOutput[resultIndex]);";
		if (previousLayer_activationLayer==2)
			replaceString=  "fmax ( inputs[globalId] , convert_half(0.0f) )*(gradOutput[resultIndex]);";
		if (previousLayer_activationLayer==3)
			replaceString=  "(1 - inputs[globalId] * inputs[globalId])*(gradOutput[resultIndex]);";
		if (previousLayer_activationLayer==4)
			replaceString=  "(0.66667f * (1.7159f - 1 / 1.7159f * inputs[globalId] * inputs[globalId]) )*(gradOutput[resultIndex]);";
