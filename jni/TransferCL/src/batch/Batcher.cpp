...
#else
		CLWrapper * lossWrap=temp->getLossWrapper();
		CLWrapper * nbWrap=temp->getNbRightWrapper();
		half *loss=lossWrap->map_ZeroCopyObject_ReadFlagHalf();
		int *nb=nbWrap->map_ZeroCopyObject_ReadFlagInt();
		epochResult.loss=HalfToFloat(loss[0]);
		epochResult.numRight=nb[0];
		lossWrap->unMap_ZeroCopyObject_ReadFlagHalf(loss);
		nbWrap->unMap_ZeroCopyObject_ReadFlagInt(nb);
#endif
