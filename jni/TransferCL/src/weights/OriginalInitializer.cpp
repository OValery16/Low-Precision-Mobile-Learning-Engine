...
VIRTUAL void OriginalInitializer::initializeWeightsHalf(int numWeights, half *weights, int fanin) {
#if TRANSFERCL_VERBOSE == 1
LOGI( "DeepCL/src/weights/OriginalInitializer.cpp: initializeWeights");
#endif


	half rangesize = FloatToHalf(sqrt(12.0f / (half)fanin)) ;
    for(int i = 0; i < numWeights; i++) {
        half uniformrand = FloatToHalf(RandomSingleton::uniform());
        half weight = rangesize * (uniformrand - 0.5f);
        weights[i] = weight;
    }
}
VIRTUAL void OriginalInitializer::initializeBiasHalf(int numBias, half *bias, int fanin) {
#if TRANSFERCL_VERBOSE == 1
LOGI( "DeepCL/src/weights/OriginalInitializer.cpp: initializeBias");
#endif
