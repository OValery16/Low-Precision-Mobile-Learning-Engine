...
VIRTUAL void UniformInitializer::initializeWeightsHalf(int numWeights, half *weights, int fanin) {
#if TRANSFERCL_VERBOSE == 1
LOGI( "DeepCL/src/weights/UniformInitializer.cpp: initializeWeights");
#endif


	half range = FloatToHalf(multiplier / (half)fanin);
    for(int i = 0; i < numWeights; i++) {
        half uniformrand = FloatToHalf(RandomSingleton::uniform() * 2.0f - 1.0f);
        weights[i] = range * uniformrand;
    }
}
VIRTUAL void UniformInitializer::initializeBiasHalf(int numBias, half *bias, int fanin) {
#if TRANSFERCL_VERBOSE == 1
LOGI( "DeepCL/src/weights/UniformInitializer.cpp: initializeBias");
#endif


    half range = FloatToHalf(multiplier / (float)fanin);
    for(int i = 0; i < numBias; i++) {
        half uniformrand = FloatToHalf(RandomSingleton::uniform() * 2.0f - 1.0f);
        bias[i] = range * uniformrand;
    }
