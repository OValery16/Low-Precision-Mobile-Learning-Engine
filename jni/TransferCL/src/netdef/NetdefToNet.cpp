...
void NetdefToNet::createFullyConnectedLayer(int batchsize,NeuralNet *net, std::string layerDef, std::string activation_layerDef,  WeightsInitializer *weightsInitializer,bool isLastB) {
	...
	net->addLayer(FullyConnectedMaker::instance()->numPlanes(numPlanes)->imageSize(1)->biased(biased)->weightsInitializer(weightsInitializer)->batchSize(batchsize)->activationLayer(activation_layerDef)->isLast(isLastB)->iteration(net->iteration));
LOGI("--------------------done");
