...
void *inc_x(void *x_void_ptr0)
{
		...
		s_param *x_void_ptr=(s_param*)x_void_ptr0;
		half *loss=((CLWrapper *)x_void_ptr->a)->map_ZeroCopyObject_ReadFlagHalf();
		int *nb=((CLWrapper *)x_void_ptr->b)->map_ZeroCopyObject_ReadFlagInt();
		x_void_ptr->nb_a=HalfToFloat(loss[0]);
		x_void_ptr->nb_b=nb[0];
		((CLWrapper *)x_void_ptr->a)->unMap_ZeroCopyObject_ReadFlagHalf(loss);
		((CLWrapper *)x_void_ptr->b)->unMap_ZeroCopyObject_ReadFlagInt(nb);
	#endif
return NULL;
