...
SGDState::SGDState(EasyCL *cl, int numWeights) :
	...
	lastUpdateWrapper= (CLWrapper*) cl->wrap(numWeights, test2);
	lastUpdateWrapper->createZeroCopyObject_ReadWriteFlag_OnDevice();
#else
	half * test3=0;

	lastUpdateWrapperHalf= (CLWrapper*) cl->wrap(numWeights, test3);
	lastUpdateWrapperHalf->createZeroCopyObject_ReadWriteFlag_OnDevice();
#endif
}
