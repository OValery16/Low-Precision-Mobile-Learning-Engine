...
VIRTUAL void BackwardGpuNaive::backwardHalf(int batchSize,
        CLWrapper *inputDataWrapper, CLWrapper *gradOutputWrapper, CLWrapper *weightsWrapper,
        CLWrapper *gradInputWrapper) {
#if MEASURE_BACKWARD_PROP
    LOGI("BackwardGpuNaive");
...
if ( not setupHalf){
    ...
    kernelHalf->in(batchSize);

    kernelHalf->in(gradOutputWrapper);
	kernelHalf->in(weightsWrapper);
    kernelHalf->out(gradInputWrapper);
    kernelHalf->in(inputDataWrapper);
}
...
/////////////////////
    string kernelSourceHalf="   ";

    TemplatedKernel builderHalf(cl);


    string identifierHalf="BackpropNaive2Half";
	...
	ConfigManager*binariesManagerHalf=new ConfigManager( cl->absolutePath/*path,binaryFileRep*/);
	bool compiledHalf;
	string filepathHalf="default";
	if (not binariesManagerHalf->alreadyCompiledKernel("calcGradInputHalf","",identifierHalf,filepathHalf)){

		inferenceBackwardHalf(kernelSourceHalf);
		setActivationFunctionHalf(&builderHalf);
		setupBuilderBackwardHalf(&builderHalf);
	}

	this->kernelHalf = builderHalf.buildKernel(
			identifierHalf,
		   "BackpropNaiveHalf",
		   kernelSourceHalf.c_str(),
		   "calcGradInputHalf",
		   false
	);
}
...
void BackwardGpuNaive::inferenceBackwardHalf(string &kernelSource) {

	string kernelSource2="#pragma OPENCL EXTENSION cl_khr_fp16 : enable\n\n"
			"void kernel calcGradInputHalf(\n"
		    "        const int batchSize,\n"
		    "        const __global half* restrict gradOutput, global half *weights, global half *gradInput,const __global half* restrict inputs) {\n"
		    "    int globalId = get_global_id(0);\n"
		    "\n"
		    "    const int upstreamImage2dId = globalId ;\n"
		    "\n"
		    ...
		    "                          + outRow)\n"
		    "                          + outCol;\n"
		    "                half thisError = gradOutput[resultIndex];\n"
		    "                int thisWeightIndex = (( outPlane * {{gInputPlanes}}\n"
		    "                                    + upstreamPlane) * {{gFilterSize}}\n"
		    "                                    + filterRow) * {{gFilterSize}}\n"
		    "                                    + filterCol;\n"
		    "                half thisWeight = weights[thisWeightIndex];\n"
		    "                half thisWeightTimesError = thisWeight * thisError;\n"
		    "                sumWeightTimesOutError += thisWeightTimesError;\n"
		    "            }\n"
		    "        }\n"
		    "    }\n"
	...
	string kernelSource3="#pragma OPENCL EXTENSION cl_khr_fp16 : enable\n\n"
				"void kernel calcGradInputHalf(\n"
				"        const int batchSize,\n"
				"        const __global half* restrict gradOutput, global half *weights, global half *gradInput,const __global half* restrict inputs) {\n"
			    "    int globalId = get_global_id(0);\n"
				...
				"    #pragma unroll\n"
			    "    for (int outPlane = 0; outPlane < {{gNumFilters}}; outPlane++) {\n"
			    "                half thisError = gradOutput[( n * {{gNumFilters}}+ outPlane)];\n"
			    "                half thisWeight = weights[(( outPlane * {{gInputPlanes}}+ upstreamPlane))];\n"
			    "                sumWeightTimesOutError += thisWeight * thisError;\n"
			    "    }\n"
			    "    gradInput[globalId] = {{gActivationFunction}};\n"
			    "}\n"
...
void  BackwardGpuNaive::setActivationFunctionHalf(TemplatedKernel *builder){
	...
	string replaceString=" ";
		...
		if (dim.previousLayer_activationLayer==3)
			replaceString=  "(1 - inputs[globalId] * inputs[globalId])*sumWeightTimesOutError";
		if (dim.previousLayer_activationLayer==4)
			replaceString=  "(convert_half(0.66667f) * (convert_half(1.7159f) - 1 / convert_half(1.7159f) * inputs[globalId] * inputs[globalId]) )*sumWeightTimesOutError";
		if (dim.previousLayer_activationLayer==5)
			replaceString=  "(inputs[globalId] * (1 - inputs[globalId]) )*sumWeightTimesOutError";
		if (dim.previousLayer_activationLayer==6)
			replaceString=  "(select(inputs[globalId]+1,1,fmax ( inputs[globalId] , 0 )))*sumWeightTimesOutError";
...
void BackwardGpuNaive::setupBuilderBackwardHalf(TemplatedKernel *builder) {
