...
//(olivier) add half floating point support and 8bit buffer (char)
#include "CLHalfWrapper.h"
#include "CLCharWrapper.h"
...
CLCharWrapper *EasyCL::wrap8Bit(int N, unsigned char *source) {
	//olivier
    return new CLCharWrapper(N, source, this);
}
...
//	cl_int status = clGetDeviceInfo(device, CL_DEVICE_PARTITION_MAX_SUB_DEVICES,
//	                             sizeof(maxSubDevices), &maxSubDevices, NULL);
//	LOGI("max subDev %d",maxSubDevices);
//	//////////////////////////
...
CLKernel *EasyCL::buildKernelFromStringOnlineBuilding(string operation,string source, string kernelname, string options, string sourcefilename){
//LOGI("--------------------------- the kernel hasn't been prebuilt; we need to build it in OpenCL");
