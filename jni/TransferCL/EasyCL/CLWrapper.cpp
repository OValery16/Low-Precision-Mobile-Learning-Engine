...
void CLWrapper::copyToDevice_ZeroCopyObject_ReadWriteFlag(const void * buffer) {
    if(!onHost) {
    	LOGI("not on host: problem");
        throw std::runtime_error("copyToDevice(): not on host - copyToDevice_ZeroCopyObject_WriteFlag");
    }
    if(!onDevice) {
        LOGI("not onDevice: we have to create it");
        createZeroCopyObject_ReadWriteFlag_OnDevice();
    }
...
void CLWrapper::copyToDevice_ZeroCopyObject_ReadWriteFlagHalf(const void * buffer) {
    if(!onHost) {
    	LOGI("not on host: problem");
        throw std::runtime_error("copyToDevice(): not on host");
    }
    if(!onDevice) {
        LOGI("not onDevice: we have to create it-copyToDevice_ZeroCopyObject_WriteFlagHalf");
        createZeroCopyObject_ReadWriteFlag_OnDevice();
    }
	half * mappedMatReflector = (half *)clEnqueueMapBuffer (*(cl->queue),devicearray,CL_TRUE,  CL_MAP_WRITE,0,getElementSize() * N, 0, NULL, NULL, NULL);
	const float * bufferf=(const float*)buffer;
	for(int i=0; i<N;i++){
		mappedMatReflector[i]=FloatToHalf(bufferf[i]);
	}
	clEnqueueUnmapMemObject(*(cl->queue),devicearray, mappedMatReflector, 0, NULL, NULL);
...
void CLWrapper::copyToDevice_ZeroCopyObject_WriteFlagHalf(const void * buffer) {
    if(!onHost) {
    	LOGI("not on host: problem");
        throw std::runtime_error("copyToDevice(): not on host");
    }
...
void CLWrapper::copyToDevice_ZeroCopyObject_WriteFlagFloat(const void * buffer) {
    if(!onHost) {
    	LOGI("not on host: problem");
        throw std::runtime_error("copyToDevice(): not on host");
    }
    if(!onDevice) {
        LOGI("not onDevice: we have to create it-copyToDevice_ZeroCopyObject_WriteFlagFloat");
        createZeroCopyObject_WriteFlag_OnDevice();
    }
	float * mappedMatReflector = (float *)clEnqueueMapBuffer (*(cl->queue),devicearray,CL_TRUE,  CL_MAP_WRITE,0,getElementSize() * N, 0, NULL, NULL, NULL);
	const half * bufferf=(const half*)buffer;
	for(int i=0; i<N;i++){
		mappedMatReflector[i]=HalfToFloat(bufferf[i]);
	}
	clEnqueueUnmapMemObject(*(cl->queue),devicearray, mappedMatReflector, 0, NULL, NULL);
...
int * CLWrapper::map_ZeroCopyObject_ReadFlagInt() {
	// note olivier: do not forget to unmap the buffer
	int * mappedMatReflector = (int *)clEnqueueMapBuffer (*(cl->queue),devicearray,CL_TRUE,  CL_MAP_READ,0,getElementSize() * N, 0, NULL, NULL, NULL);

return mappedMatReflector;
}
void CLWrapper::unMap_ZeroCopyObject_ReadFlagInt(int *ptr) {
	// note olivier: do not forget to unmap the buffer
...
half * CLWrapper::map_ZeroCopyObject_ReadFlagHalf() {
	// note olivier: do not forget to unmap the buffer
	half * mappedMatReflector = (half *)clEnqueueMapBuffer (*(cl->queue),devicearray,CL_TRUE,  CL_MAP_READ,0,getElementSize() * N, 0, NULL, NULL, NULL);

return mappedMatReflector;
}
void CLWrapper::unMap_ZeroCopyObject_ReadFlagHalf(half *ptr) {
	// note olivier: do not forget to unmap the buffer

	clEnqueueUnmapMemObject(*(cl->queue),devicearray, ptr, 0, NULL, NULL);

}
void CLWrapper::unMap_ZeroCopyObject_WriteFlagHalf(half *ptr) {
	// note olivier: do not forget to unmap the buffer
...
half * CLWrapper::map_ZeroCopyObject_WriteFlagHalf() {
	// note olivier: do not forget to unmap the buffer
...
void CLWrapper::unMap_ZeroCopyObject_WriteFlagHalf(float *ptr) {
	// note olivier: do not forget to unmap the buffer
...
unsigned char * CLWrapper::map_ZeroCopyObject_WriteFlag8Bit() {
	// note olivier: do not forget to unmap the buffer

	unsigned char * mappedMatReflector = (unsigned char *)clEnqueueMapBuffer (*(cl->queue),devicearray,CL_TRUE,  CL_MAP_WRITE,0,getElementSize() * N, 0, NULL, NULL, NULL);
return mappedMatReflector;
}

void CLWrapper::unMap_ZeroCopyObject_WriteFlag8Bit(unsigned char *ptr) {
	// note olivier: do not forget to unmap the buffer
...
unsigned char * CLWrapper::map_ZeroCopyObject_ReadFlag8Bit() {
	// note olivier: do not forget to unmap the buffer
	unsigned char * mappedMatReflector = (unsigned char *)clEnqueueMapBuffer (*(cl->queue),devicearray,CL_TRUE,  CL_MAP_READ,0,getElementSize() * N, 0, NULL, NULL, NULL);

return mappedMatReflector;
}
void CLWrapper::unMap_ZeroCopyObject_ReadFlag8Bit(unsigned char *ptr) {
	// note olivier: do not forget to unmap the buffer
