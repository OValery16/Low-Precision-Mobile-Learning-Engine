...
void TrainModel::go(ConfigTraining config) {
	    ...
	    trainAllocateN=config.batchSize;
	   	...
	   	//    long allocateSize = (long)Ntrain * numPlanes * imageSize * imageSize;
			...
			LOGI("Ntest %d plane %d imagesSize %d",testLoader->getN(),testLoader->getPlanes(),testLoader->getImageSize());
			...
			for(int i =0;i<20;i++){
				LOGI("testData[%d]=%f",i,testData[i]);
			}
	    ...
	    net->iteration=config.numTrain/config.batchSize;
	    ...
	    //iteration=trainAllocateN/config.numTrain;
...
//		#if TRANSFER ==1
	    ...
	    cl->finish();
LOGI("config.weightsStoreFile %s",config.weightsStoreFile.c_str());
